...
<body>
  <main class="app">
    ...
    <section class="table-wrap" aria-label="德州撲克牌桌">
    ...
    <section class="controls">
      <div class="hand-readout">
        ...
        <div class="hint" id="hintText">你會坐在下方座位，其餘玩家由電腦操作。每局按鈕位置會輪流移動。</div>
      </div>
  ...
  <script>
    ...
    const STORAGE_KEY = "texas-poker-game-state-v1";
    ...
    const PRESENCE_APP = "texas-poker";
    const AI_DELAY = {
      normal: [220, 460],
      folded: [60, 130]
    };
    ...
    function loadRaiseBuilderCollapsed() {
      try {
        return localStorage.getItem("texas_poker_raise_builder_collapsed") === "1";
      } catch (error) {
        return false;
      }
    }
    ...
    function setRaiseBuilderCollapsed(collapsed) {
      ...
      try {
        localStorage.setItem("texas_poker_raise_builder_collapsed", collapsed ? "1" : "0");
      } catch (error) {}
    }

    function makePresenceId() {
      try {
        const key = "texas_poker_presence_id";
    ...
    function exportLog() {
      ...
      link.href = url;
      link.download = `texas-poker-hand-log-${stamp}.txt`;
    ...
    function drawPotential(p) {
      ...
      const handSuits = new Set(p.hand.map(card => card.suit));
      let flushDraw = 0;
      Object.entries(suitCounts).forEach(([suit, count]) => {
        if (!handSuits.has(suit)) return;
    ...
    function render() {
      ...
      if (hero && hero.hand.length) {
        const best = bestHand([...hero.hand, ...state.board]);
        els.handTitle.textContent = state.board.length ? `你的牌型：${HAND_NAMES[best.score[0]]}` : `你的手牌：${hero.hand.map(c => rankText(c.rank) + c.suit).join(" ")}`;
      }
    ...
    function startHand() {
      ...
      for (let cardNo = 0; cardNo < 2; cardNo++) {
    ...
    function estimateStrength(p) {
      ...
      if (state.board.length === 0) {
        ...
        let value = (high - 2) / 12 * .38 + (low - 2) / 12 * .16;
        if (a.rank === b.rank) value += .32 + (a.rank - 2) / 12 * .16;
        if (a.suit === b.suit) value += .07;
        if (Math.abs(a.rank - b.rank) <= 2) value += .06;
        if (high >= 13) value += .04;
        return Math.min(.97, value + Math.random() * .08);
      }
      const best = bestHand([...p.hand, ...state.board]).score;
    ...
    function showdown() {
      ...
      const ranked = contenders.map(p => ({
        ...
        best: bestHand([...p.hand, ...state.board])
      }));
    ...
    function runPokerRuleChecks() {
      const boardPair = [card(7, "♠"), card(7, "♥"), card(10, "♦"), card(4, "♣"), card(2, "♠")];
      const kingKicker = bestHand([card(14, "♣"), card(13, "♦"), ...boardPair]).score;
      const queenKicker = bestHand([card(14, "♦"), card(12, "♣"), ...boardPair]).score;
      console.assert(
        compareScore(kingKicker, queenKicker) > 0,
        "同一對必須繼續比較角牌：A/K/10 應勝過 A/Q/10。",
        kingKicker,
        queenKicker
      );

      const exactTieA = bestHand([card(14, "♣"), card(3, "♦"), ...boardPair]).score;
      const exactTieB = bestHand([card(14, "♦"), card(3, "♣"), ...boardPair]).score;
      console.assert(
        compareScore(exactTieA, exactTieB) === 0,
        "只有五張最佳牌完全相同才應該 chop。",
        exactTieA,
        exactTieB
      );
