...
const SOURCES = [
    'taipei' => [
        'base' => 'https://tcgbusfs.blob.core.windows.net/blobbus/',
        'name' => '台北市',
    ],
    'newtaipei' => [
        'base' => 'https://tcgbusfs.blob.core.windows.net/ntpcbus/',
        'name' => '新北市',
    ],
];
const TAIPEI_BOUNDS = [
...
function fetch_json_gz(string $file, int $ttl, string $cacheDir, string $source = 'taipei'): array
{
    if (!isset(SOURCES[$source])) {
        throw new RuntimeException("Unknown source {$source}");
    }

    $cacheFile = $cacheDir . '/' . $source . '_' . preg_replace('/[^a-z0-9_.-]/i', '_', $file) . '.json';
    ...
    $context = stream_context_create([
        'http' => [
            'timeout' => 12,
            'header' => "User-Agent: taipei-bus-gen-art/1.0\r\n",
        ],
    ]);
    $compressed = file_get_contents(SOURCES[$source]['base'] . $file, false, $context);
    if ($compressed === false) {
        throw new RuntimeException("Cannot fetch {$file}");
    }

    $body = gzdecode($compressed);
    if ($body === false) {
        throw new RuntimeException("Cannot decode {$file}");
    }
...
function get_routes(string $cacheDir): array
{
    ...
    foreach (array_keys(SOURCES) as $source) {
        $routeData = fetch_json_gz('GetRoute.gz', 86400, $cacheDir, $source);
        $updatedAt = max_update_time($updatedAt, $routeData['EssentialInfo']['UpdateTime'] ?? null);

        foreach (($routeData['BusInfo'] ?? []) as $route) {
            $rawId = (int) ($route['Id'] ?? 0);
            $rawLiveId = (int) ($route['pathAttributeId'] ?? $rawId);
            if ($rawId <= 0) {
                continue;
            }

            $id = route_key($source, $rawId);
            $liveId = route_key($source, $rawLiveId);
            $name = trim((string) ($route['nameZh'] ?? ''));
            if (!isset($routesById[$id])) {
                $routesById[$id] = [
                    ...
                    'liveId' => $liveId,
                    'liveIds' => [],
                    'name' => $name !== '' ? $name : (string) $rawId,
                    'departure' => (string) ($route['departureZh'] ?? ''),
                    'destination' => (string) ($route['destinationZh'] ?? ''),
                    'provider' => (string) ($route['providerName'] ?? ''),
                ];
            }

            if ($rawLiveId > 0 && !in_array($liveId, $routesById[$id]['liveIds'], true)) {
                $routesById[$id]['liveIds'][] = $liveId;
            }
        }
    }

    $routes = array_values($routesById);
...
function get_live(string $cacheDir): array
{
    ...
    foreach (array_keys(SOURCES) as $source) {
        $liveData = fetch_json_gz('GetBusData.gz', 8, $cacheDir, $source);
        $updatedAt = max_update_time($updatedAt, $liveData['EssentialInfo']['UpdateTime'] ?? null);

        foreach (($liveData['BusInfo'] ?? []) as $bus) {
            $routeId = (int) ($bus['RouteID'] ?? 0);
            $routeKey = route_key($source, $routeId);
            if ($routeFilter !== [] && !in_array($routeKey, $routeFilter, true) && !in_array((string) $routeId, $routeFilter, true)) {
                continue;
            }

            $lat = (float) ($bus['Latitude'] ?? 0);
            $lng = (float) ($bus['Longitude'] ?? 0);
            if (!in_taipei_bounds($lat, $lng)) {
                continue;
            }
            ...
            $buses[] = [
                ...
                'lat' => $lat,
                'lng' => $lng,
                'speed' => (float) ($bus['Speed'] ?? 0),
                'azimuth' => (int) ($bus['Azimuth'] ?? 0),
                'direction' => (string) ($bus['GoBack'] ?? ''),
                'dataTime' => (string) ($bus['DataTime'] ?? ''),
            ];
...
{
    [$source, $rawRouteId] = parse_route_key((string) $routeId);
    if ($rawRouteId <= 0) {
        return ['ok' => true, 'routeId' => 0, 'lines' => []];
    }

    $shapeData = fetch_json_gz('GetBusShape.gz', 86400, $cacheDir, $source);
    $lines = [];

    foreach ($shapeData as $shape) {
        if ((int) ($shape['RouteID'] ?? 0) !== $rawRouteId) {
            continue;
        }

        $points = parse_linestring((string) ($shape['wkt'] ?? ''));
        if ($points !== []) {
            $lines[] = [
                'goBack' => (string) ($shape['GoBack'] ?? ''),
                'points' => $points,
            ];
        }
    }
    ...
    return [
        'ok' => true,
        'routeId' => route_key($source, $rawRouteId),
        'lines' => $lines,
    ];
}

function route_key(string $source, int $routeId): string
{
...
function parse_route_key(string $routeId): array
{
    if (str_contains($routeId, ':')) {
        ...
        return [isset(SOURCES[$source]) ? $source : 'taipei', (int) $rawId];
    }

    return ['taipei', (int) $routeId];
}
...
function parse_linestring(string $wkt): array
{
    if (!preg_match('/^LINESTRING\s*\((.+)\)$/i', trim($wkt), $match)) {
        return [];
    }
...
function in_taipei_bounds(float $lat, float $lng): bool
{
...
<head>
    ...
    <title>台北公車即時動態 Gen Art</title>
    <meta name="description" content="以台北公車即時資料生成會發光的城市移動軌跡。">
...
<div id="app">
    ...
    <section class="hud" aria-label="台北公車即時控制台">
        <div class="topline">
            <h1>台北公車即時動態</h1>
            <div class="status">
                <div id="clock">讀取中</div>
                <div id="sourceTime">Data.Taipei</div>
            </div>
        ...
        <div class="hud-body" id="hudBody">
            <div class="controls">
                <input id="routeSearch" list="routeOptions" type="search" placeholder="搜尋路線，例如 307、紅2、南京" aria-label="搜尋路線">
...
const FAVORITES_STORAGE_KEY = 'taipeiBusFavoriteRoutes';
const HUD_COLLAPSED_STORAGE_KEY = 'taipeiBusHudCollapsed';
const IS_EMBEDDED = (() => {
    try {
...
function popupFor(bus) {
    const direction = bus.direction === '0' ? '去程' : (bus.direction === '1' ? '返程' : '方向未定');
...
async function loadLive() {
    ...
    updateMetrics(data);
    document.getElementById('sourceTime').textContent = data.updatedAt || 'Data.Taipei';
}
